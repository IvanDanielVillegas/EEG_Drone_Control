...
x = keras.layers.Conv1D(6, 50, activation='relu',input_shape=input_shape)(inputs)#16,250
x = keras.layers.MaxPool1D(pool_size=2, strides = 2, padding = 'same')(x)
x = keras.layers.Conv1D(16, 250, activation='relu',input_shape=input_shape)(x)#16,80
x = keras.layers.MaxPool1D(pool_size=2, strides = 2, padding = 'same')(x)
x = keras.layers.Flatten()(x)
x = keras.layers.Dense(250,activation = 'relu')(x)#250
x = keras.layers.Dropout(0.4)(x)#0.2
...
model.load_weights('pesos_fin1.h5') ### Cargar los pesos 'pesosI80.h5' 'pesosF2.h5'
...
################################################################################################
    ...
    def updater_EEG(self):
        global data, colors,board,stm, LowF,HighF, model, switch, altura, fila, prueba, Orden
        ...
        if mini == 1000:
            ...
            print("switch>>>>"+str(switch)+"..."+str(prediccion)+"---"+str(Orden))
...
def Orden_Drone():
    global Orden
    while True:
         print("Orden al drone..."+str(Orden))
         if Orden == 0:
             ui.Despegar()
...
#    timerDrone = QtCore.QTimer()
#    timerDrone.timeout.connect(ui.Orden_Drone)
#    timerDrone.start(1000)
    
    hilo_Drone = threading.Thread(target=Orden_Drone) 
    hilo_Drone.daemon = True
    hilo_Drone.start()
    time.sleep(3)
